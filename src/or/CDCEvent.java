...
import cn.wizool.android.com.google.code.or.binlog.impl.event.AbstractBinlogEventV4;
...
public class CDCEvent {
    ...
    private String timestamp ;//事件发生的时间戳[MySQL服务器的时间]
    private String timestampReceipt;//Open-replicator接收到的时间戳[CDC执行的时间戳]
    ...
    private void init(final BinlogEventV4 be){
        ...
        this.timestamp = simpleDateFormat.format(new Date(header.getTimestamp()));
        this.eventType = header.getEventType();
        this.serverId = header.getServerId();
        this.timestampReceipt = simpleDateFormat.format(header.getTimestampOfReceipt());
        System.out.println(header.getTimestampOfReceipt());
	...
	public String getTimestampReceipt() {
		return timestampReceipt;
	}

	public void setTimestampReceipt(String timestampReceipt) {
